...
def main():
    ...
    llm = ChatOpenAI(openai_api_key=api_key, model_name="gpt-4.1-mini", temperature=0.0)
    ...
    for idx, qa in enumerate(questions, start=1):
        ...
        cands = base_ret.get_relevant_documents(q)
        texts = [d.page_content for d in cands]
        rerank = co_client.rerank(model="rerank-v3.5", query=q, documents=texts, top_n=1).results[0]
        best_score, best_chunk = rerank.relevance_score, texts[rerank.index]
        print(f"CRAG: best chunk idx={rerank.index}, score={best_score:.2f}")
        ...
        prompt = f"{SYSTEM_PREFIX}Question: {q}\nContext:\n{context}\n\nAnswer concisely in Ukrainian:"
